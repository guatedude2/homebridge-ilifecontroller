...
var getSWVersion = function() {
	...
	superagent.get("http://"+that.credentials+that.ip_address+"/api/info").timeout(60000).end(function(error, response) {
		if (error) {
...
ILifeController.prototype = {
	setPowerState: function(powerOn, callback) {
		if (powerOn) {
			...
			if (!this.dock_on_stop){
				...
				superagent.get("http://"+that.credentials+that.ip_address + "/api/status").end(function(error, response) {
					if (error) {
	...
	getPowerState: function(callback) {
		var url = "http://"+this.credentials+this.ip_address + "/api/status";
	...
	identify: function(callback) {
		...
		superagent.get("http://"+this.credentials+this.ip_address + "/api/info").end(function(error, response) {
			if (error) {
